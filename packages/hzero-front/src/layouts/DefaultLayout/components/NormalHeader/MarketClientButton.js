...
function MarketClientButton(props) {
  ...
  const queryShowButtonResult = () => {
    ...
    fetch(`${API_HOST}${HZERO_ADM}/v1/market/config/icon`, {
      headers: {
  ...
  return (
    <StyledMarketClientButton onClick={() => props.history.push('/market-client')}>
      <Tooltip title={intl.get('hzero.common.view.title.marketClient').d('应用市场')}>
...
export default withRouter(MarketClientButton);
